# Unified Branch - Medium Branch System Functional Specification

## 1.0 Overview ##

Phase 2 of Unified Branch supports a network design for small, medium, and large branch sites.  This SFS document discusses the medium branch design. 

## 2.0 Medium Branch High-Level Design ##

The high-level layout of the medium branch design is shown below.

**Figure 1. Medium Branch - High Level Overview**

![Medium Branch High Level](images/medium_branch_high_level_no_mg.png)

The medium branch network design consists of the following components: 

 - Two Cisco secure routers in an active/standby configuration, each with up to 2 WAN uplinks for redundancy.

 - A single cloud managed Layer 2 switch or switch stack, where the number of switches supported in the stack is the maximum number of switches supported in a stack configuration for the model of switch deployed.  

> :information_source:
> If more than a single switch or stack is required due to physical distance limitations (100 meters with wired Ethernet) within the branch, Cisco recommends transitioning to a hierarchical LAN design by implementing a distribution layer switch or switch stack, with one or more access-layer switches or switch stacks.  The Phase 2 large branch design is targeted to support one such hierarchical LAN design in which the distribution layer switch stack operates at Layer 3.  Future updates to the Unified Branch designs may discuss hierarchical medium and/or large branch designs which implement Layer 2 only LAN designs. 

 - WLAN Access Points (APs), where the maximum number of APs deployed within the site depends upon the requirements of the medium branch site. 

## 3.0 Physical Connectivity ##

- This section was last updated on 03/11/2026 to modify the native VLAN on switch ports connecting to APs from VLAN 1 to VLAN 999.  This is due to not having an API (and therefore no Terraform Provider resource) to change the native VLAN on APs themselves from VLAN 1 to VLAN 999.  Note that it is not necessarily inline with Cisco best practices to have the native VLAN of the AP set to VLAN 1 while the native VLAN of the switch port to which the AP is trunked set to VLAN 999. Using the Alternative Management Interface (AMI) is another option that may need to be looked at in the future to get around the issue of not having a programmatic way of simply moving the management VLAN from VLAN 1 to VLAN 999. 
...
Figure 1 above shows both landline WAN service providers connected to both Cisco secure routers.  In reality, there is only a single physical Ethernet handoff from each WAN service provider, which needs to connect to both the primary and secondary Cisco secure routers.  This results in the following two options:
 ...
 - Option 1:  Creating two VLANs (VLANs 901 & 902) on the L2 switch/switch stack.

 - Option 2:  Implementing separate non-Meraki managed switches to "split" the Ethernet handoff from each landline WAN service provider.
 
 ### 3.1 Option 1 ###

 With Option 1, VLANS 901 and 902 configured on the medium branch L2 switches simply to provide L2 passthrough connectivity from the WAN service provider physical Ethernet handoff to the Gigabit Ethernet ports (WAN1 and WAN2 respectively) of the Cisco secure routers, as shown in the figure below.

**Figure 2. Medium Branch - Physical Connectivity Option 1**

![Medium Branch Option 1](images/medium_branch_connectivity_option1.png)

For Option 1, the following would be the port configurations based on each switch (assuming a 2 switch stack):

**Table 1. Option 1 Switch_01 Port Connections**

| Switch_01 | Connected To | Switch Port Configuration |
| ----------| ------------ | ------------------------- |
...
| Ports 9-12 | Dedicated for APs | Trunk Port - VLANs 1, 10, 20, 30, 40, 50, 999 (native) |
| Ports 13-24/48 | Dedicated for Devices | Access Ports |

**Table 2. Option 1 Switch_02 Port Connections**

| Switch_02 | Connected To | Switch Port Configuration |
| ----------| ------------ | ------------------------- |
...
Infrastructure reserved ports and ports dedicated for APs which are unused should be shutdown.
 ...
 Option 2 is for customers who are uncomfortable with using the medium branch L2 switches to provide both the WAN and LAN connectivity.  This is often due to potential security concerns regarding possible misconfiguration or in the event that the switch platforms are returned to a factory default configuration, or simply due to additional complexity when initially standing up the branch.  
 
 With Option 2, a pair of separate non-Meraki dashboard managed (IOS XE autonomous) switches are deployed within the medium branch simply to provide L2 passthrough connectivity from the WAN service provider physical Ethernet handoff to the Gigabit Ethernet ports (WAN1 and WAN2 respectively) of the Cisco secure routers, as shown in the figure below.

**Figure 3. Medium Branch - Physical Connectivity Option 2**

![Medium Branch Option 2](images/medium_branch_connectivity_option2.png)

For Option 2, the following would be the port configurations based on each switch (assuming a 2 switch stack):

**Table 3. Option 2 Switch_01 Port Connections**
...
**Table 4. Option 2 Switch_02 Port Connections**
...
**Table 5. Option 2 External_switch_01 Port Connections**
...
**Table 6. Option 2 External_switch_02 Port Connections**
...
- This section was updated on 03/11/2026 to modify the native VLAN on switch ports connecting to APs from VLAN 1 to VLAN 999.  This is due to not having an API (and therefore no Terraform Provider resource) to change the native VLAN on APs themselves from VLAN 1 to VLAN 999.  

- This section was updated on 02/14/2026 to reflect only 2 options for physical layout due to removal of 5G fixed wireless access (MG52) in a MultiWAN Backup Uplink configuration for Phase 2 of Unified Branch.
...
The *app_ports* template within the [*templates-appliance.nac.yaml*](https://github.com/netascode/nac-branch/blob/main/data/templates-inventory.nac.yaml) file within the Cisco Unified Branch GitHub Repository will need to be modified to include VLAN 40 (PCI VLAN) as follows:
...
The *templates-wan-uplinks.nac.yaml* file will need to be updated to remove all references to IPv6.  IPv6 is not supported with both the Multi-WAN Backup Uplink interface or in HA (warm spare) configuration.

#### 3.5.2 Updates to Secure Router SVI Definitions within the app_vlans Template ####

This section last updated on 04/03/2026 to add reserved IP address ranges for the VLAN 999 DHCP server.  Comments indicate the use of fixed IP addresses and reserved IP addresses for the management interface(s) and SVI interface, depending upon whether the type of L2 switch or switch stack (native IOS XE, CS, or MS).
...
DHCP relay should be implemented for VLANs 10, 20, and 30 for Unified Branch Phase 2.  This was commented out due to issues with the Terraform provider for Unified Branch Phase 1.

Finally, for VLAN 999 (Infra), fixed IP address assignments corresponding to the Layer 2 switch stack and APs was deemed desired.  Again, due to issues with the Terraform porvider, this was commented out for Unified Branch Phase 1, and should be included in Unified Branch Phase 2.  It should be noted that a best practice would be to manually configure IP addresses for infrastructure equipment in order to guarantee that the IP addresses do not change from a management perspective.  However, currently there is no API (and hence no Terraform Provider resource) for manually configuring the IP addresses of individual switches and APs.
...
For the MX67 and MX68, all models are supported per decision in the 12/11/2025 technical meeting, however the integrated wireless and/or cellular functions are not supported.  Also, a switch or switch stack is required for LAN connectivity within the branch.

Unified Branch Phase 2 will support the follow additional Cisco secure router platforms.

- C8121-MX  (Note:  Is this platform going to be available by release of Phase 2?)
- C8455-MX
...
This section discusses only the additional services provided by the Cisco secure router, switch, and APs within the medium branch design for Phase 2 of Unified Branch.  These services are added on top of the services already supported by the small branch design from Unified Branch Phase 1, which are assumed to also be supported by the medium branch design 

### 5.1 Redundant WAN Routers – Active/Standby ###
For Unified Branch Phase 2, it is mandatory for the medium branch design to include redundant Cisco secure routers in an active/standby configuration.
...
**Figure 5. Medium Branch Hardware-Level Redundancy**

![Medium Branch Redundancy](images/medium_branch_redundancy.png)

To accommodate the medium branch design, a new template called *medium_branch_inventory* will need to be added to [*templates-inventory.nac.yaml*](https://github.com/netascode/nac-branch/blob/main/data/templates-inventory.nac.yaml) file within the existing Cisco Unified Branch GitHub Repository.  The new template is discussed in section *3.5 Medium Branch Inventory Config Template - Router, Switch, and AP Configurations* of this document.
...
For the medium branch design the following are the configuration for the parameters:
...
### 5.2 ZTNA / Adaptive Policy with External Radius Server ###
...
![Adaptive Policy with Radius Use Case](images/adaptive_policy_with_radius_use_case.png)
 ...
 #### 5.2.1 Adaptive Policy Group Configuration ####
...
#### 5.2.2 Adaptive Policies Configuration ####
...
The following table shows the adaptive policy to be created for Phase 2 of Unified Branch.
...
#### 5.2.3. Adaptive Policy Settings ####
...
#### 5.2.4 YAML Configuration - Adaptive Policy Groups, Policies, and Settings ####

The following is an example of the YAML configuration for creation of the Adaptive Policy Groups & Adaptive Policies, as well as the application of the Adaptive Policies to the networks (via the list of network names under the *settings_enabled_networks* parameter) used in the example for the small, medium, and L2 large branch designs for Phase 2 of Unified Branch.

This will need to be added to the existing [*org_global.nac.yaml*](https://github.com/netascode/nac-branch/data/org_global.nac.yaml) file within the Cisco Unified Branch GitHub Repository.
              ...
              - name: !env org_name
                ...
                adaptive_policy:
                  ...
                  settings_enabled_networks:
                    - ${small_branch_01_network_name}       # How do we specify which networks without using variables?
...
#### 5.2.5 Modifications to Switch Ports from Phase 1 to Pass SGT Tags and Switch Port Assignment Changes  ####
...
#### 5.2.6 Modifications to Appliance LAN Ports from Phase 1 to Pass SGT Tags  ####

This section was updated on 02/19/2026.
...
The Terraform Provider will need to pick up support for the *peer_sgt_capable* and *adaptive_policy_group_id* parameters from Early Access APIs to support Adaptive Policy for Phase 2 of Unified Branch.

#### 5.2.7 Modifications to Appliance AutoVPN Configuration from Phase 1 to Pass SGT Tags  ####
...
### 5.3 SSE / Cisco Secure Access Cloud Integration ##
...
#### 5.3.3 Third Party VPN Tunnel Health Check ####
...
- *tests* - List of Attributes (optional) containing an array of tests to be created.  The Terraform provider indicates this can only be configured during resource creation and cannot be changed afterwards.
  ...
  - *template_user_inputs_tenant* - String containing the tenant value. 
...
###  5.6 Modifications from Unified Branch Phase 1 ###

This section discusses changes to the Unified Branch design from Phase 1.

#### 5.6.1 Switch Configuration Modifications ####
...
##### 5.6.1.1 802.1x Switch Access Policy #####
...
##### 5.6.1.2 STP Priority Switch Setting #####
...
##### 5.6.1.3 Switch Stacks Configuration #####

The GitHub repository for Unified Branch Phase 1 contained a switch stack definition for the small branch.  The intent of this configuration was that a single switch deployment would be a stack of only 1 switch.  Testing has indicated that an error will be gnereated if a switch stack contains less than two switches.  Therefore, the *stp* and *switch_stacks* sections of the *switch* template also needs to be modified.  For small and medium branch designs, the fastest way of mitigating this issue is to include both switch and switch stack parameters under the *stp* definition as follows:
...
The YAML will default to a switch stack configuration for the small and medium branches.  If the customer has a switch stack configuration, then he/she will simply need to add the necessary switch names in the list under *devices* within the *switch_stacks* section.  If the customer does not have a switch stack configuration, then he/she will need to comment out the *stacks* parameter, uncomment the *switches* parameter, and add the serial number of the branch switch.  The customer will also have to comment out the *switch_stacks* section.  

This configuration is not ideal, but a workable solution for Phase 2.  However, if the customer has different branches - some with switches and others with switch stacks - this will force the customer to duplicate the entire *switches* template.  This will result in duplication of configuration for the *access_settings*, *settings*, *mtu*, *routing_multicast*, *dscp_to_cos*, *qos_rules*, and *storm_control* sections which may be common across all branches. Hence it should be considered if the *stp* and *switch_stacks* sections should be split out into separate one or two separate templates. 

Note also, that the configuration shown above will not work for the large branch design.  The large branch design will need the configuration similar to the following to account for the distribution and one or more access switches or switch stacks:
...
Again, the issue to be resolved is how to accomodate the customer having different branches - small, medium, and large - in which the large branches could also have different numbers of access switches and/or switch stacks.  A single *switches* template will not likely be flexible enough. 

##### 5.6.1.4 Storm Control Settings #####
...
#### 5.6.2 Sync VPN Firewall Policy with CVD ####

This section added 02/20/2026.
...
#### 5.6.3 Sync L3, L7, and Firewalled Services Policy with CVD ####

This section added 02/20/2026.

Within the Phase 1 Unified Branch release, the example shown for L3, L7, and firewalled services policy was not consistent with the CVD.  The reasoning behind this was that the customer would have to determine the specific firewall rules specific to their deployment.  L3 and L7 outbound firewall rules control what traffic is allowed between VLANs configured on the secure router, as well as DIA traffic outbound to the Internet.  

However, the marketing message has been that Cisco is taking the best practices design from the CVD and encoding it in the YAML files within Branch as Code as well as within Workflows.  Hence, there is an inconsistency with the marketing message and what is being delivered within the automation.  Further, if the desire is to train an AI agent to return specific configuration, based on the Branch as Code YAML files, corresponding to the design within the CVD, then the L3, L7, and firewalled services policy rules within the Branch as Code YAML files should match the CVD design.

The following changes to the *app_fw* template within the [*templates-appliance.nac.yaml*](https://github.com/netascode/nac-branch/blob/main/data/templates-appliance.nac.yaml) file are suggested synchronize the L3, L7, and firewalled services policy with the CVD.
        ...
        meraki:
          template:
            networks:
              ...
              # and firewalled services template.
              #
              ...
              #   - Allow all devices on the Data VLAN (VLAN 10), access to the 
              #     IP addresses of the printers on the shared services / 
              #     IoT VLAN.  All Data VLANs across branches are assumed
              #     to use the IP addressing of 10.10.x.0/24, where x is the
              #     specific branch.
                  ...
                  firewall:
                          l3_firewall_rules:
                              ...
                              - comment: "Local Print Access"
                                ...
                                source_cidr: "10.10.0.0/16"
                                source_port: any
                                # The destination_cidr values for this rule 
                                # needs to be specified as a string of variables,
                                # "${branch_printer_1},${branch_printer2}".
                                # Otherwise, the template cannot be used across
                                # multiple branches.   
                                destination_cidr: "10.30.1.101,10.30.1.102"
                                destination_port: any
                              - comment: "Allow DIA"
                                policy: allow
                                protocol: any
...
#### 5.6.4 Sync WAN Traffic Shaping and Uplink Selection Policy with CVD ####

This section updated 03/31/2026 to remove the VPN exclusion for ICMP for destination 8.8.8.8/32.

This section added 02/23/2026.
...
#### 6.1.1 Medium Branch High-Level Design with Fixed Wireless Access Support ####

The high-level layout of the medium branch design with fixed wireless access support is shown below.

**Figure 15. Medium Branch with Fixed Wireless Access Support - High Level Overview**

![Medium Branch High Level](images/medium_branch_high_level.png)
...
Fixed wireless access support adds the option for the customer/partner to choose to implement a 3rd WAN interface in a MultiWAN Backup Uplink configuration using an MG52 Cellular Gateway.  This results in the following two additional physical connectivity options for the Unified Branch medium branch design.
 ...
 With Option 3, VLANS 901 and 902 are again configured on the medium branch L2 switches simply to provide L2 passthrough connectivity from the WAN service provider physical Ethernet handoff to the Gigabit Ethernet ports (WAN1 and WAN2 respectively) of the Cisco secure routers. Additionally, the customer / partner has chosen to implement a 3rd WAN interface in a MultiWAN Backup Uplink configuration using an MG52 Cellular Gateway.  This is shown in the figure below.

**Figure 16. Medium Branch - Physical Connectivity Option 3**

![Medium Branch Option 3](images/medium_branch_connectivity_option3.png)

When implmenting a 3rd WAN interface on the MX85/95/105 platforms, the pairing of ports 1 & 3 and ports 2 & 4 for WAN connectivity no longer applies.  Port 4 is used for the backup uplink.  This requires WAN1 to use Port 1 and WAN2 to use Port 2 on the MX85/95/105 platforms.  Since Ports 1 & 2 are SFP ports, this will require the customer / partner to purchase copper SFP modules when connecting RJ45 Ethernet ports on the medium branch switch stack, or if the medium branch switch stack supports SFP ports - optionally fiber SFP modules. 

For Option 3, the following would be the port configurations based on each switch (assuming a 2 switch stack):

**Table 9. Option 3 Switch_01 Port Connections**
...
**Table 10. Option 3 Switch_02 Port Connections**
...
**Table 11. Option 3 MG52 Port Connections**
...
Infrastructure reserved ports and ports dedicated for APs which are unused should be shutdown. Note also, that in a MultiWAN Backup Uplink configuration, Port 3 of the MX85/95/105 is disabled and cannot be used.
 ...
 With Option 4, a pair of separate non-Meraki managed (IOS XE autonomous) switches are deployed within the medium branch simply to provide L2 passthrough connectivity from the WAN service provider physical Ethernet handoff to the Gigabit Ethernet ports (WAN1 and WAN2 respectively) of the Cisco secure routers.  Additionally, the customer / partner has chosen to implement a 3rd WAN interface in a MultiWAN Backup Uplink configuration using an MG52 Cellular Gateway.  This is shown in the figure below.

**Figure 17. Medium Branch - Physical Connectivity Option 4**

![Medium Branch Option 4](images/medium_branch_connectivity_option4.png)

For Option 4, the following would be the port configurations based on each switch (assuming a 2 switch stack):

**Table 12. Option 4 Switch_01 Port Connections**
...
**Table 13. Option 4 Switch_02 Port Connections**
...
**Table 14. Option 4 External_switch_01 Port Connections**
...
**Table 15. Option 4 External_switch_02 Port Connections**
...
**Table 16. Option 4 MG52 Port Connections**
| MG52 Physical Port | Connected To |
| ------------------ | ------------ |
| Port 1 | appliance_01, Port 4 |
| Port 2 | appliance_02, Port 4 |
...
- This section was last updated on 03/11/2026 to modify the native VLAN on switch ports connecting to APs from VLAN 1 to VLAN 999.  This is due to not having an API (and therefore no Terraform Provider resource) to change the native VLAN on APs themselves from VLAN 1 to VLAN 999.  
...
To accommodate the medium branch design, a new template called *medium_branch_inventory* will need to be added to [*templates-inventory.nac.yaml*](https://github.com/netascode/nac-branch/blob/main/data/templates-inventory.nac.yaml) file within the existing Cisco Unified Branch GitHub repository.  The new template will support an MG52 cellular gateway as follows:
       ...
       - name: medium_branch_inventory
         devices:
          ...
          #
          ~
          # Additional configuration for the appliances, switches, and APs not shown here.
          ~
	        #
          #  The below code does the following:
          #
          ...
          #
          #  For Unified Branch Phase 2 neither fixed IP address assignments
          #  or port-forwarding rules is required.  Therefore, these sections 
          #  can be either  omitted or commented out.
          #
          #  For Unified Branch Phase 2, it will be assumed the customer / 
          #  partner will configure the SIMs as part of the MG52 onboarding
          #  process, using a dedicated device attached to the MG52.  Therefore,
          #  the SIMs section is not included within the following YAML
          #  configuration.  For information on how to add this to the YAML configuration
          #  please refer to the Cellular SIMS section of the Meraki data model 
          #  within the netascode site:
          ...
          - name: ${cellular_01_name}
            ...
            # configuration transferred to the Phase 2 production release.
...
Meraki Access Manager has been removed for Phase 2 due to APIs not being available to select Meraki Access Manager for authentication and authorization within the Switching and Wireless sections of the Meraki dashboard. However the use case for Adaptive Policy with external Radius server will be included in Phase 2, as discussed in Section *5.2 ZTNA / Adaptive Policy with External Radius Server* of this document. 

The following figure shows the use case for Adaptive Policy with Access Manager for Phase 2 of Unified Branch.

**Figure 19. Medium Branch Adaptive Policy with Access Manager Use Case**
